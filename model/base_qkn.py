"""Decoder-only transformer with QK-normed attention and RoPE.

A compact Llama-style stack (RMSNorm pre-norm, SwiGLU MLP, rotary positions).
Query/key RMSNorm keeps attention logits well-scaled during training. The
config below is overridden per run by files under configs/.
"""
...
class RalphBase(nn.Module):
    """Decoder-only transformer: token ids (B, T) -> logits (B, T, vocab_size)."""
